...
require_relative 'GregorianLunarCalendar_types'


class GregorianLunarCalendarSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GregorianLunarCalendarHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GregorianLunarCalendarHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GregorianLunarCalendarFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GregorianLunarCalendarUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GregorianLunarCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GregorianLunarCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GregorianLunarCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GregorianLunarCalendarError.new(
        "#{op}_allow",
        "GregorianLunarCalendarSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue GregorianLunarCalendarError => err
    ...
    if fetched.is_a?(Hash)
      status = GregorianLunarCalendarHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GregorianLunarCalendarError.new(
        "graphql_error", "GregorianLunarCalendarSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GregorianLunarCalendarSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
