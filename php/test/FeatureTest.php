...
// GregorianLunarCalendar SDK feature test
//
...
class FtCtrl extends GregorianLunarCalendarControl
{
...
class FtClient
{
    ...
    private GregorianLunarCalendarUtility $utility;

    public function __construct(GregorianLunarCalendarUtility $utility, array $options)
    {
    ...
    public function get_utility(): GregorianLunarCalendarUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GregorianLunarCalendarContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GregorianLunarCalendarContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GregorianLunarCalendarContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GregorianLunarCalendarConfig::make_feature($name);
    ...
    private static function buildUrl(GregorianLunarCalendarSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GregorianLunarCalendarError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GregorianLunarCalendarResponse($fetched);
        }
    ...
    private function fail(GregorianLunarCalendarContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GregorianLunarCalendarContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
