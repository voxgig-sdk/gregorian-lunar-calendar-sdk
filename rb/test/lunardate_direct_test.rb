...
class LunardateDirectTest < Minitest::Test
  def test_direct_load_lunardate
    setup = lunardate_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-lunardate", setup[:live] ? "live" : "unit")
    if _should_skip
...
def lunardate_direct_setup(mockres)
