...
//     (\`new GregorianLunarCalendarSDK(...)\` / \`GregorianLunarCalendarSDK.test(...)\` become
//     \`GregorianLunarCalendarSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'GregorianLunarCalendarSDK'
...
const TEST_SEED = {"entity":{"lunardate":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GregorianLunarCalendarSDK(...)\` and any
// \`GregorianLunarCalendarSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GregorianLunarCalendarSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GregorianLunarCalendarSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
