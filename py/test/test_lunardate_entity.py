    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GREGORIAN_LUNAR_CALENDAR_TEST_LUNARDATE_ENTID JSON to run live")
        client = setup["client"]
        ...
        lunardate_ref01_data_dt0_loaded = lunardate_ref01_ent.load(lunardate_ref01_match_dt0, None)
...
def _lunardate_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GREGORIAN_LUNAR_CALENDAR_TEST_LUNARDATE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("GREGORIAN_LUNAR_CALENDAR_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("GREGORIAN_LUNAR_CALENDAR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("GREGORIAN_LUNAR_CALENDAR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
