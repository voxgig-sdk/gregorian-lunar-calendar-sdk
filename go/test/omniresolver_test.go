...
import (
	...
	omni "github.com/voxgig-sdk/gregorian-lunar-calendar-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GregorianLunarCalendarError); is {
				out := map[string]any{
					"name":    "GregorianLunarCalendarError",
...
func omniCtx(arg any, client *sdk.GregorianLunarCalendarSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GregorianLunarCalendarSDK); is {
			client = live
