...
require("gregorian-lunar-calendar_types")
...
local GregorianLunarCalendarSDK = {}
GregorianLunarCalendarSDK.__index = GregorianLunarCalendarSDK
...
GregorianLunarCalendarSDK._make_feature = _make_feature


function GregorianLunarCalendarSDK.new(options)
  local self = setmetatable({}, GregorianLunarCalendarSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GregorianLunarCalendarSDK:options_map()
...
function GregorianLunarCalendarSDK:get_utility()
  return Utility.copy(self._utility)
end


function GregorianLunarCalendarSDK:get_root_ctx()
  return self._rootctx
end


function GregorianLunarCalendarSDK:prepare(fetchargs)
...
function GregorianLunarCalendarSDK:direct(fetchargs)
...
function GregorianLunarCalendarSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function GregorianLunarCalendarSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "GregorianLunarCalendarSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function GregorianLunarCalendarSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GregorianLunarCalendarSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GregorianLunarCalendarSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GregorianLunarCalendarSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GregorianLunarCalendarSDK.new(sdkopts)
  sdk.mode = "test"
...
return GregorianLunarCalendarSDK
