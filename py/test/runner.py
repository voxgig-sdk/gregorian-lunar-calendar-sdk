# GregorianLunarCalendar SDK test runner
...
class GregorianLunarCalendarTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GregorianLunarCalendarTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GregorianLunarCalendarTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GregorianLunarCalendarTestRunner.getenv("GREGORIAN_LUNAR_CALENDAR_TEST_LIVE")
        override = GregorianLunarCalendarTestRunner.getenv("GREGORIAN_LUNAR_CALENDAR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GregorianLunarCalendarTestRunner.getenv(key)
        ...
        explain = GregorianLunarCalendarTestRunner.getenv("GREGORIAN_LUNAR_CALENDAR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GregorianLunarCalendarTestRunner._test_control is not None:
            return GregorianLunarCalendarTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GregorianLunarCalendarTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GregorianLunarCalendarTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GregorianLunarCalendarTestRunner.load_test_control()
...
def load_env_local():
    GregorianLunarCalendarTestRunner.load_env_local()


def env_override(m):
    return GregorianLunarCalendarTestRunner.env_override(m)


def entity_data(v):
    return GregorianLunarCalendarTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GregorianLunarCalendarTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GregorianLunarCalendarTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GregorianLunarCalendarTestRunner.load_test_control()


def live_client_options():
    return GregorianLunarCalendarTestRunner.live_client_options()


def live_delay_ms():
    return GregorianLunarCalendarTestRunner.live_delay_ms()
