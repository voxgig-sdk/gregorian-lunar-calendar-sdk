...
AQL-driven command-line client **and** interactive REPL for the GregorianLunarCalendar
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gregorian-lunar-calendar-cli)
make build

# 2. See usage (words, entities, env vars)
./gregorian-lunar-calendar-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gregorian-lunar-calendar-cli
   ```
...
Run with no arguments to open a REPL (prompt `gregorian-lunar-calendar>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `lunardate`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/gregorian-lunar-calendar-cli`. |
...
(`./gregorian-lunar-calendar-cli <expr>`) and interactively (the REPL), and expressions compose the
