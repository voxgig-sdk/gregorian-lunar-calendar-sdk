...
module GregorianLunarCalendarUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GregorianLunarCalendarResponse.new({})
    ...
    if fetch_err
      ...
      response = GregorianLunarCalendarResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GregorianLunarCalendarResponse.new(fetched)
    else
