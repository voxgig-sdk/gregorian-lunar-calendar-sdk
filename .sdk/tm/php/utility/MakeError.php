...
class GregorianLunarCalendarMakeError
{
    public static function call(?GregorianLunarCalendarContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GregorianLunarCalendarContext([], null);
        }
        $op = $ctx->op ?? new GregorianLunarCalendarOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GregorianLunarCalendarResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GregorianLunarCalendarError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GregorianLunarCalendarError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof GregorianLunarCalendarError) {
