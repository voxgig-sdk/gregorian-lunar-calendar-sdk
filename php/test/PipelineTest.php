...
// GregorianLunarCalendar SDK pipeline test
//
...
{
    private static function utility(): GregorianLunarCalendarUtility
    {
        return new GregorianLunarCalendarUtility();
    }
    ...
    private static function ctx(array $over = []): GregorianLunarCalendarContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GregorianLunarCalendarContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GregorianLunarCalendarError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GregorianLunarCalendarMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GregorianLunarCalendarMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GregorianLunarCalendarMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GregorianLunarCalendarSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GregorianLunarCalendarMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GregorianLunarCalendarResponse([]),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        [, $err] = GregorianLunarCalendarMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec([]),
            'response' => null,
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        [, $err] = GregorianLunarCalendarMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec(['step' => 's']),
            'response' => new GregorianLunarCalendarResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        [, $err] = GregorianLunarCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec(['step' => 's']),
            'response' => new GregorianLunarCalendarResponse(self::resp(200, ['v' => 1])),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        [, $err] = GregorianLunarCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec(['step' => 's']),
            'response' => new GregorianLunarCalendarResponse(self::resp(200, ['v' => 2])),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GregorianLunarCalendarMakeResponse::call($ctx);
    ...
    {
        $preset = new GregorianLunarCalendarResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec([]),
            'response' => new GregorianLunarCalendarResponse([]),
            'result' => new GregorianLunarCalendarResult([]),
        ...
        [$got, $err] = GregorianLunarCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GregorianLunarCalendarResult([])]);
        [, $err] = GregorianLunarCalendarMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GregorianLunarCalendarSpec([]), 'result' => null]);
        [, $err] = GregorianLunarCalendarMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GregorianLunarCalendarSpec(['step' => 's']),
            'result' => new GregorianLunarCalendarResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GregorianLunarCalendarMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GregorianLunarCalendarSpec(['step' => 's']),
            'result' => new GregorianLunarCalendarResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GregorianLunarCalendarOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GregorianLunarCalendarMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GregorianLunarCalendarResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GregorianLunarCalendarSpec([]),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GregorianLunarCalendarMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GregorianLunarCalendarMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GregorianLunarCalendarError('boom', 'boom');
        $utility->fetcher = function (GregorianLunarCalendarContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GregorianLunarCalendarSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GregorianLunarCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GregorianLunarCalendarContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GregorianLunarCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GregorianLunarCalendarResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GregorianLunarCalendarMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GregorianLunarCalendarContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GregorianLunarCalendarSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GregorianLunarCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GregorianLunarCalendarResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GregorianLunarCalendarSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GregorianLunarCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GregorianLunarCalendarMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GregorianLunarCalendarMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GregorianLunarCalendarResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GregorianLunarCalendarDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GregorianLunarCalendarResult(['ok' => false])]);
        $this->expectException(GregorianLunarCalendarError::class);
        GregorianLunarCalendarDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GregorianLunarCalendarResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GregorianLunarCalendarMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GregorianLunarCalendarMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GregorianLunarCalendarBaseFeature();
        $b = new GregorianLunarCalendarBaseFeature();
        GregorianLunarCalendarFeatureAdd::call($ctx, $a);
        GregorianLunarCalendarFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GregorianLunarCalendarBaseFeature
    {
        $f = new GregorianLunarCalendarBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GregorianLunarCalendarFeatureAdd::call($ctx, self::named_feature('a'));
        GregorianLunarCalendarFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        GregorianLunarCalendarFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        GregorianLunarCalendarFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        GregorianLunarCalendarFeatureAdd::call($ctx, $replace);
        ...
        GregorianLunarCalendarFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GregorianLunarCalendarContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GregorianLunarCalendarSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GregorianLunarCalendarPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GregorianLunarCalendarPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GregorianLunarCalendarResponse(['headers' => null]),
            'result' => new GregorianLunarCalendarResult([]),
        ]);
        GregorianLunarCalendarResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GregorianLunarCalendarResultBody::call($ctx);
