...
class LunardateEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = lunardate_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "lunardate." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GREGORIAN_LUNAR_CALENDAR_TEST_LUNARDATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    lunardate_ref01_data = nil
    if lunardate_ref01_data_raw.length > 0
...
def lunardate_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "lunardate", "LunardateTestData.json")
  ...
  entid_env_raw = ENV["GREGORIAN_LUNAR_CALENDAR_TEST_LUNARDATE_ENTID"]
  ...
  if env["GREGORIAN_LUNAR_CALENDAR_TEST_LIVE"] == "TRUE"
    ...
    client = GregorianLunarCalendarSDK.new(Helpers.to_map(merged_opts))
  end
