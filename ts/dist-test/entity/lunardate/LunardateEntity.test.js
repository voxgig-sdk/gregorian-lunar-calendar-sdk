...
(0, node_test_1.describe)('LunardateEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when GREGORIAN_LUNAR_CALENDAR_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('GREGORIAN_LUNAR_CALENDAR_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.GregorianLunarCalendarSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'lunardate.' + op, live))
                return;
        }
        ...
        let lunardate_ref01_data = Object.values(setup.data.existing.lunardate)[0];
        // LOAD
        const lunardate_ref01_ent = client.Lunardate();
        const lunardate_ref01_match_dt0 = {};
        const lunardate_ref01_data_dt0 = (await lunardate_ref01_ent.load(lunardate_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != lunardate_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/lunardate/LunardateTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.GregorianLunarCalendarSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['lunardate01', 'lunardate02', 'lunardate03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        client = new __1.GregorianLunarCalendarSDK(merge([
...
//# sourceMappingURL=LunardateEntity.test.js.map