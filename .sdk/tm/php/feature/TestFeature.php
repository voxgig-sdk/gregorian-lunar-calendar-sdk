...
// GregorianLunarCalendar SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class GregorianLunarCalendarTestFeature extends GregorianLunarCalendarBaseFeature
{
    ...
    public function init(GregorianLunarCalendarContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GregorianLunarCalendarContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GregorianLunarCalendarParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GregorianLunarCalendarContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GregorianLunarCalendarContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GregorianLunarCalendarParam::call($ctx, $k);
