...
//    only - a typed GregorianLunarCalendarContext there would make every ctx
...
final class GregorianLunarCalendarOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GregorianLunarCalendarContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new GregorianLunarCalendarSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new GregorianLunarCalendarResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new GregorianLunarCalendarError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new GregorianLunarCalendarResponse($clean['response']);
    ...
    public static function err_from_map($m): ?GregorianLunarCalendarError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GregorianLunarCalendarError(is_string($code) ? $code : '', $msg);
    }
