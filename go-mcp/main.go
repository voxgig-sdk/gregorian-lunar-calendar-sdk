// Command GregorianLunarCalendar-mcp is an MCP server exposing the GregorianLunarCalendar
// SDK as MCP tools.
//
// Two tools are registered:
//
//	GregorianLunarCalendar_list  — list records of an entity
//	GregorianLunarCalendar_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: GREGORIAN_LUNAR_CALENDAR_APIKEY carries the API key and
	// GREGORIAN_LUNAR_CALENDAR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GREGORIAN_LUNAR_CALENDAR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GREGORIAN_LUNAR_CALENDAR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGregorianLunarCalendarSDK(opts)
