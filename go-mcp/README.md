# gregorian-lunar-calendar-mcp

[MCP](https://modelcontextprotocol.io) server exposing the GregorianLunarCalendar SDK as
two agent tools — `gregorian-lunar-calendar_list` and `gregorian-lunar-calendar_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gregorian-lunar-calendar-mcp)
make build
...
claude mcp add --scope user gregorian-lunar-calendar \
  -- /absolute/path/to/gregorian-lunar-calendar-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./gregorian-lunar-calendar-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// gregorian-lunar-calendar_list: first page of records
{ "entity": "lunardate" }
{ "entity": "lunardate", "query": { } }

// gregorian-lunar-calendar_load: one record by id
{ "entity": "lunardate", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gregorian-lunar-calendar-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user gregorian-lunar-calendar \
     -- "$PWD"/dist/*/gregorian-lunar-calendar-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `gregorian-lunar-calendar_list` and `gregorian-lunar-calendar_load` tools now appear
   in new sessions. Ask the agent to *"list lunardate using gregorian-lunar-calendar"*
   and it calls `gregorian-lunar-calendar_list` with `{"entity":"lunardate"}`.
...
### Call the `gregorian-lunar-calendar_list` tool
...
### Call the `gregorian-lunar-calendar_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `gregorian-lunar-calendar_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `gregorian-lunar-calendar_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GREGORIAN_LUNAR_CALENDAR_APIKEY` | API key sent with every request. |
| `GREGORIAN_LUNAR_CALENDAR_BASE` | Optional override of the API base URL. |
...
```sh
./gregorian-lunar-calendar-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gregorian-lunar-calendar_load","arguments":{"entity":"lunardate","query":{"id":1}}}}'
```
