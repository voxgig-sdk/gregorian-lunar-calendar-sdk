...
local LunardateEntity = {}
LunardateEntity.__index = LunardateEntity


function LunardateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LunardateEntity)
  self._name = "lunardate"
  self._client = client
...
function LunardateEntity:get_name()
  return self._name
end


function LunardateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LunardateEntity.new(self._client, opts)
end
...
function LunardateEntity:mark_deleted()
  self._deleted = true
end


function LunardateEntity:deleted()
  return true == self._deleted
end


function LunardateEntity:data_set(args)
  if args ~= nil then
...
function LunardateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LunardateEntity:match_set(args)
  if args ~= nil then
...
function LunardateEntity:match_get()
...
function LunardateEntity:stream(action, args, callopts)
...
function LunardateEntity:_run_op(ctx, post_done)
  local utility = self._utility
