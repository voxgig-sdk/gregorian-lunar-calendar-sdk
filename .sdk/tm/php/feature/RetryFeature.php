...
// GregorianLunarCalendar SDK retry feature
...
class GregorianLunarCalendarRetryFeature extends GregorianLunarCalendarBaseFeature
{
    ...
    public function init(GregorianLunarCalendarContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GregorianLunarCalendarContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GregorianLunarCalendarContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GregorianLunarCalendarContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
