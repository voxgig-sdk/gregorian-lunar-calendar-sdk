...
// GregorianLunarCalendar SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GregorianLunarCalendarOmni::makeStructRunner(
    ...
    {
        return GregorianLunarCalendarOmni::objmodel(GregorianLunarCalendarOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return GregorianLunarCalendarOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return GregorianLunarCalendarOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GregorianLunarCalendarOmni::structfix($now, Struct::undef()))
                        !== json_encode(GregorianLunarCalendarOmni::structfix($val, Struct::undef()))) {
