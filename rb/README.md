# GregorianLunarCalendar Ruby SDK

The Ruby SDK for the GregorianLunarCalendar API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Lunardate(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.GregorianLunarCalendar(nil).load(
  { "id" => "test01" }, nil
)
...
```ruby
client = GregorianLunarCalendarSDK.test(testopts, sdkopts)
```
...
The main module (`GregorianLunarCalendar_sdk`) exports the SDK class
