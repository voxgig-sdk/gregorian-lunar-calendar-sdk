...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("GREGORIANLUNARCALENDAR_TEST_LIVE") == "TRUE" ||
		os.Getenv("GREGORIANLUNARCALENDAR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("GREGORIANLUNARCALENDAR_TEST_EXPLAIN"); explain != "" {
